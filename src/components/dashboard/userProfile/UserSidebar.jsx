...
const UserSidebar = () => {
  ...
  const handleDeleteExperience = async (experienceId) => {
    ...
    try {
      ...
      setWorkExperiences(prev => prev.filter(exp => exp._id !== experienceId));
    } catch (error) {
  ...
  const handleSaveSocialLink = async (linkData) => {
    try {
      if (editingSocialLink) {
        ...
        const response = await userProfileAPI.updateSocialLink(editingSocialLink._id, linkData);
        if (response?.data) {
    ...
    <>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-4 md:p-5 lg:p-6 relative">
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          ...
          <button 
            onClick={isEditingContact ? handleCancelContactEdit : handleEditContact}
                ...
                <>
                  {profileData.email && (
                    <div className="flex items-center gap-2.5 sm:gap-3">
                      ...
                      <span className="text-sm sm:text-sm text-gray-600 truncate">{profileData.email}</span>
                    </div>
                  )}
                  {profileData.phone && (
                    <div className="flex items-center gap-2.5 sm:gap-3">
                      ...
                      <span className="text-sm sm:text-sm text-gray-600">{profileData.phone}</span>
                    </div>
                  )}
                  ...
                  {(!profileData.email && !profileData.phone && !profileData.location) && (
                    <p className="text-sm sm:text-sm text-gray-500 text-center py-4">No contact information available. Click edit to add.</p>
                  )}
                </>
              )}
            </>
          ) : (
            <p className="text-sm sm:text-sm text-gray-500 text-center py-4">No profile found. Please complete your profile setup.</p>
          )}
        </div>
        ...
        <div className="mt-4 sm:mt-6">
          ...
          {isLoadingSocialLinks ? (
            <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gray-200 animate-pulse"></div>
              ))}
            </div>
            ...
            <div className="space-y-2">
              {socialLinks.map((link, index) => {
                ...
                return (
                  <div key={link._id || `social-link-${index}`} className="flex items-center justify-between group">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-[#ECEFF6] flex items-center justify-center text-[#240457]">
                        {IconComponent ? <IconComponent /> : <FiMail className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900 capitalize">{link.platformName}</p>
                        <a 
                          href={link.url} 
                          target="_blank" 
        ...
        {isLoading ? (
          ...
          <div className="border border-gray-200 rounded-lg p-2.5 sm:p-3 flex items-center justify-between mb-3 sm:mb-4">
            <div className="flex items-center gap-2.5 sm:gap-3">
              ...
              <div>
                <p className="text-sm sm:text-sm font-medium text-gray-900">Resume</p>
                <p className="text-sm sm:text-sm text-gray-500">
                  {profileData.updatedAt ? `Updated ${new Date(profileData.updatedAt).toLocaleDateString()}` : 'Available'}
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              ...
              <button className="text-red-500 hover:text-red-700 p-1" onClick={handleDeleteResume}>
          ...
          <div className="border border-dashed border-gray-300 rounded-lg p-4 sm:p-6 text-center mb-3 sm:mb-4">
            ...
            <p className="text-sm sm:text-sm text-gray-500">No resume uploaded yet</p>
          </div>
        )}
        ...
        <div className="relative">
          ...
          >
            <span className="hidden sm:inline">
              {isUploadingResume ? 'Uploading...' : profileData?.resumeUrl ? 'Update Resume/Portfolio' : 'Upload Resume/Portfolio'}
            </span>
            <span className="sm:hidden">
              {isUploadingResume ? 'Uploading...' : profileData?.resumeUrl ? 'Update Resume' : 'Upload Resume'}
            </span>
            {!isUploadingResume && (
              <svg
                width="14"
      ...
      {/* Job Matches Section */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-4 md:p-5 lg:p-6">
        <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-3 sm:mb-4">Job Matches</h3>
        ...
        {isLoadingJobMatches ? (
          ...
          <div className="space-y-2 sm:space-y-3">
            {jobMatches.slice(0, 3).map((job, index) => {
              const colors = ['bg-orange-500', 'bg-teal-500', 'bg-blue-500', 'bg-purple-500', 'bg-green-500'];
              const bgColor = colors[index % colors.length];
              const companyInitial = job.businessProfileId?.businessName?.charAt(0).toUpperCase() || 'J';
              
              return (
                <div key={job._id} className="flex items-center justify-between p-2.5 sm:p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer">
                  <div className="flex items-center gap-2.5 sm:gap-3">
                    <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg ${bgColor} flex items-center justify-center text-white font-bold text-sm sm:text-base`}>
                      {companyInitial}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm sm:text-base font-medium text-gray-900 truncate">{job.jobTitle}</p>
                      <p className="text-xs sm:text-sm text-gray-500">{job.businessProfileId?.businessName || 'Company Name'}</p>
                    </div>
                  </div>
                  <svg
                    width="16"
          ...
          <div className="text-center py-6">
            ...
            <p className="text-xs text-gray-400 mt-1">Complete your profile to get better matches</p>
          </div>
        )}
